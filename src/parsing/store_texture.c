...
/*   Updated: 2025/04/23 11:04:42 by gasroman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	store_texture(char **split, t_data *data)
{
	...
	if (!ft_strncmp(split[0], "NO", 3) && !data->no)
		data->no = ft_strdup(split[1]);
	else if (!ft_strncmp(split[0], "SO", 3) && !data->so)
		data->so = ft_strdup(split[1]);
	else if (!ft_strncmp(split[0], "EA", 3) && !data->ea)
		data->ea = ft_strdup(split[1]);
	else if (!ft_strncmp(split[0], "WE", 3) && !data->we)
		data->we = ft_strdup(split[1]);
	else
		return (printf("Texture Error3\n"), 1);
	return (0);
}
